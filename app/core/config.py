...
class Settings(BaseSettings):
    ...
    # ---- 云端 Embedding（火山引擎 Ark 豆包向量模型）----
    # 注意：Embedding 与 LLM 不是同一家。LLM 用 DeepSeek，Embedding 用火山 Ark，
    # 两者 base_url / model 必须分别配置，否则 RAG 检索会调用到错误的接口而失败。
    ...
    embedding_model: str = "doubao-embedding-vision-250615"  # 豆包向量模型名（已验证可用）
    ...
    rerank_cross_encoder_model: str = "BAAI/bge-reranker-v2-m3"  # 交叉编码器模型名
